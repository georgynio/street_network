...
content = [
    ("emission_dir_weekday", "[input]", "String"),
    ...
    # activate the meteorology
    ("make_meteo", "[option]", "Bool"),
    # activate background
    ("make_background", "[background]", "Bool"),
    # background from Chimere
    ("chimere_dir", "[background]", "String"),
    ("chimout_lab", "[background]", "String"),
    ("chimere_species", "[background]", "StringList"),
    ("melch2molmass_file", "[background]", "String"),  # background from CMAQ
    ("cmaq_dir", "[background]", "String"),
    ("cmaqout_lab", "[background]", "String"),
    ("wrfout_lab", "[background]", "String"),
    ("cmaq_species", "[background]", "StringList"),
    ("cb05molmass_file", "[background]", "String"),  # background from Polair3d
...
if config.is_voc_speciated:
    ...
    if isFound == False:
        print(
            "The option is_voc_speciated is activated. To perform the speciation of VOC species, please add NMHC in the emission_species."
        )
        sys.exit()
...
if config.is_nox_speciated:
    ...
    if isFound == False:
        print(
            "The option is_nox_speciated is activated. To perform the speciation of NOx species, please add NOx in the emission_species."
        )
        sys.exit()
...
if config.is_pm10_speciated:
    ...
    if isFound == False:
        print(
            "The option is_pm10_speciated is activated. To perform the speciation of PM10 species, please add PM10 in the emission_species."
        )
        sys.exit()
    # EC
    isFound = False
    ...
    if isFound == False:
        print(
            "The option is_pm10_speciated is activated. To perform the speciation of PM10 species, please add EC in the emission_species."
        )
        sys.exit()
    # OM
    isFound = False
    ...
    if isFound == False:
        print(
            "The option is_pm10_speciated is activated. To perform the speciation of PM10 species, please add OM in the emission_species."
        )
        sys.exit()
...
if config.is_isvoc_speciated:
    ...
    if isFound == False:
        print(
            "The option is_isvoc_speciated is activated. To perform the speciation of ISVOC species, please add OM in the emission_species."
        )
        sys.exit()
# ---------
...
earth_radius_2 = 6371229.0 * 6371229.0
...
if config.option_background == 2:
    ...
    bkgd_data = read_bkgd_bin(
        config.polair3d_species,
        config.polair3d_dir,
        config.Nt_bkgd,
        config.Nx_bkgd,
        config.Ny_bkgd,
...
#################################
#                               #
#    IF BACKGROUND FROM CMAQ    #
#                               #
#################################
elif config.option_background == 4:
    cb05_spec_list = config.cmaq_species
    cmaqout_dir = config.cmaq_dir
    cmaqout_lab = config.cmaqout_lab
    wrfout_lab = config.wrfout_lab

    molar_mass_cb05 = {}

    with open(config.cb05molmass_file, "r") as f:
        for ln in f:
            #     print(ln)
            spec = ln.split()[0]
            val = float(ln.split()[1])
            molar_mass_cb05[spec] = val
    f.close()
...
for t in range(nt):
    ...
    if (current_date.weekday() >= 0) and (current_date.weekday() <= 4):
        if is_holiday(current_date, config.country_code):
            ...
            input_file = (
                config.emission_dir_weekend + "/" + config.weekend_file_prefix + hour
            )
        else:
            print("The current day is a weekday.")
            input_file = (
                config.emission_dir_weekday + "/" + config.weekday_file_prefix + hour
            )
    else:
    ...
    print(
        "Read the input data (segment coordinates and emission rates) from the file --- ",
        input_file,
    )

    street_list, node_list = read_traffic_data(
        input_file, emis_species_list, config.epsg_code
    )
    ...
    if config.is_street_merged:
        ...
        if read_lut:
            ...
            print(
                " - Number of streets after merging the same streets: %d"
                % (n_street - ntemp)
            )
        ...
        if config.is_street_manually_merged:
            ...
            print(
                " - Number of streets after manual merging of the streets: %d"
                % (n_street - ntemp - ntemp2)
            )
    ...
    if config.is_near_node_merged:
        ...
        print(
            "Number of nodes after removing the nearest nodes: ",
            (n_node - n_node1 - n_node2),
        )
    if config.is_node_manually_merged:
        n_node3 = manual_merging_node(node_list_temp)
        print(
            "Number of nodes after manually removing the nearest nodes: ",
            (n_node - n_node1 - n_node2 - n_node3),
        )
    ...
    # comentei para ver se consigo esquecer de fazer a parte meteorologica
    if config.make_meteo:
        get_meteo_data(
            config.meteo_dir,
            current_date,
            street_list_eff,
    ...
    if config.make_background:
        # Text
        if config.option_background == 1:
            background_concentration_file = config.background_concentration
            get_background_concentration(
                background_concentration_file, current_date, street_list_eff
            )
        ...
        # From Chimere output
            ...
            get_chimere_background_concentration(
                current_date,
                street_list_eff,
                melchior_spec_list,
                molar_mass_melchior2,
                chimout_dir,
                chimout_lab,
            )  # modifiquei essa linha aqui
            #      chimere_dir,chimout_lab)

        # From CMAQ output
        elif config.option_background == 4:
            get_cmaq_background_concentration(
                current_date,
                street_list_eff,
                cb05_spec_list,
                molar_mass_cb05,
    ...
    for st in street_list_eff:
        indx, indy = get_polair_ind_v2(
            st.lon_cen, st.lat_cen, x_min, Delta_x, y_min, Delta_y, Nx, Ny
        )
        lat = polair_lat[indy]
        lon = polair_lon[indx]
    ...
    for i, species in enumerate(emis_species_list):
        file_emission = output_dir + "/emission/" + species.upper() + ".bin"
        append_binary(emission_array[:, i], file_emission)
        print(f"{file_emission}\n {emission_array[:,i].shape}")
    ...
    for i, species in enumerate(emis_species_list):
        file_emission = output_dir + "/grid_emission/" + species.upper() + ".bin"
